...
HtmlElement: {"": "Element;", "%": "HTMLAppletElement|HTMLBRElement|HTMLBaseFontElement|HTMLBodyElement|HTMLCanvasElement|HTMLContentElement|HTMLDListElement|HTMLDataListElement|HTMLDetailsElement|HTMLDialogElement|HTMLDirectoryElement|HTMLDivElement|HTMLEmbedElement|HTMLFieldSetElement|HTMLFontElement|HTMLFrameElement|HTMLFrameSetElement|HTMLHRElement|HTMLHeadElement|HTMLHeadingElement|HTMLHtmlElement|HTMLIFrameElement|HTMLImageElement|HTMLKeygenElement|HTMLLabelElement|HTMLLegendElement|HTMLLinkElement|HTMLMapElement|HTMLMarqueeElement|HTMLMenuElement|HTMLMetaElement|HTMLModElement|HTMLOListElement|HTMLObjectElement|HTMLOptGroupElement|HTMLParagraphElement|HTMLPreElement|HTMLQuoteElement|HTMLScriptElement|HTMLShadowElement|HTMLSourceElement|HTMLSpanElement|HTMLStyleElement|HTMLTableCaptionElement|HTMLTableCellElement|HTMLTableColElement|HTMLTableDataCellElement|HTMLTableElement|HTMLTableHeaderCellElement|HTMLTableRowElement|HTMLTableSectionElement|HTMLTemplateElement|HTMLTitleElement|HTMLTrackElement|HTMLUListElement|HTMLUnknownElement;HTMLElement"},
...
InputElement: {"": "HtmlElement;value%", $isInputElement: true, $isEventTarget: true, "%": "HTMLInputElement"},
...
["", "playfaircipher.dart", , O, {
main: function() {
  var t1, t2, t3, t4;
  t1 = H.interceptedTypeCast(document.querySelector("#plaintextArea"), "$isTextAreaElement");
  t1.placeholder = "Put your plaintext to be encrypted here";
  t1.toString;
  t2 = C.EventStreamProvider_keyup._eventType;
  t3 = new W._ElementEventStreamImpl(t1, t2, false);
  H.setRuntimeTypeInfo(t3, [null]);
  t4 = new W._EventStreamSubscription(0, t3._target, t3._eventType, W._wrapZone(O.preCrypt$closure), t3._useCapture);
  H.setRuntimeTypeInfo(t4, [H.getRuntimeTypeArgument(t3, "_EventStream", 0)]);
  t4._tryResume$0();
  $.plaintextArea = t1;
  t1 = H.interceptedTypeCast(document.querySelector("#ciphertextArea"), "$isTextAreaElement");
  t1.placeholder = "Put your ciphertext to be decrypted here";
  t1.toString;
  t4 = new W._ElementEventStreamImpl(t1, t2, false);
  H.setRuntimeTypeInfo(t4, [null]);
  t3 = new W._EventStreamSubscription(0, t4._target, t4._eventType, W._wrapZone(O.preCrypt$closure), t4._useCapture);
  H.setRuntimeTypeInfo(t3, [H.getRuntimeTypeArgument(t4, "_EventStream", 0)]);
  t3._tryResume$0();
  $.ciphertextArea = t1;
  t1 = H.interceptedTypeCast(document.querySelector("#cipherKey"), "$isInputElement");
  t1.placeholder = "Playfair Key here";
  t1.toString;
  t2 = new W._ElementEventStreamImpl(t1, t2, false);
  H.setRuntimeTypeInfo(t2, [null]);
  t3 = new W._EventStreamSubscription(0, t2._target, t2._eventType, W._wrapZone(O.changeKey$closure), t2._useCapture);
  H.setRuntimeTypeInfo(t3, [H.getRuntimeTypeArgument(t2, "_EventStream", 0)]);
  t3._tryResume$0();
  t3 = new W._ElementEventStreamImpl(t1, C.EventStreamProvider_blur._eventType, false);
  H.setRuntimeTypeInfo(t3, [null]);
  t2 = new W._EventStreamSubscription(0, t3._target, t3._eventType, W._wrapZone(O.upperCaseKey$closure), t3._useCapture);
...
preCrypt: function(e) {
  var originalArea, targetArea, encrypt, t1;
  if (J.$eq(J.get$target$x(e), document.querySelector("#plaintextArea"))) {
    originalArea = document.querySelector("#plaintextArea");
    targetArea = document.querySelector("#ciphertextArea");
    encrypt = true;
  } else {
    originalArea = document.querySelector("#ciphertextArea");
    targetArea = document.querySelector("#plaintextArea");
    encrypt = false;
  }
  t1 = J.getInterceptor$x(originalArea);
  t1.set$value(originalArea, J.toUpperCase$0$s(t1.get$value(originalArea)));
  O.crypt(originalArea, targetArea, encrypt);
},

crypt: function(originalArea, targetArea, encrypt) {
  var t1, t2, original, targetStr, i, i0, firstLetter, t3, t4;
  t1 = document.querySelector("#ciphertextArea");
  if ((originalArea == null ? t1 == null : originalArea === t1) && C.JSInt_methods.$mod(J.get$length$asx(J.get$value$x(originalArea)), 2) === 1)
    ;
  else {
    t1 = J.getInterceptor$x(originalArea);
    if (!J.get$isEmpty$asx(t1.get$value(originalArea))) {
      t1 = J.toUpperCase$0$s(t1.get$value(originalArea));
      t2 = H.JSSyntaxRegExp_makeNative("[^a-zA-Z]*", false, true, false);
      original = H.stringReplaceAllUnchecked(t1, new H.JSSyntaxRegExp(t2, null, null), "");
      for (targetStr = "", i = 0; t1 = original.length, i < t1; i = i0) {
        i0 = i + 2;
        if (i0 > t1)
          original += "Z";
        t1 = original.length;
        if (i >= t1)
          throw H.ioore(original, i);
        firstLetter = original[i];
        t2 = i + 1;
        if (t2 >= t1)
          throw H.ioore(original, t2);
        if (firstLetter === original[t2])
          original = C.JSString_methods.substring$2(original, 0, t2) + "X" + C.JSString_methods.substring$1(original, t2);
        t1 = $.square;
        t3 = original.length;
        if (i >= t3)
          throw H.ioore(original, i);
        ...
        if (t2 >= t3)
          throw H.ioore(original, t2);
        targetStr = C.JSString_methods.$add(targetStr, t1.cryptPair$2(t4 + original[t2], encrypt));
      }
      J.set$value$x(targetArea, targetStr);
    } else
      J.set$value$x(targetArea, "");
  }
},

changeKey: function(e) {
  $.square = O.Square$($.cipherKey.value);
  O.crypt(document.querySelector("#plaintextArea"), document.querySelector("#ciphertextArea"), true);
},

upperCaseKey: function(e) {
  var t1, t2;
  t1 = J.getInterceptor$x(e);
  t2 = H.interceptedTypeCast(t1.get$target(e), "$isInputElement");
  t1 = H.interceptedTypeCast(t1.get$target(e), "$isInputElement").value;
  t1.toString;
  t2.value = t1.toUpperCase();
},

Square: {"": "Object;alphabet,columns,rows",
  generate$1: function(key) {
    var newKey, t1, t2, playfairKey, i, count, j, t3, t4;
    key.toString;
    newKey = key.toUpperCase();
    t1 = H.JSSyntaxRegExp_makeNative("j", false, true, false);
    t1 = H.stringReplaceAllUnchecked(newKey, new H.JSSyntaxRegExp(t1, null, null), "i");
    t2 = H.JSSyntaxRegExp_makeNative("J", false, true, false);
    playfairKey = H.stringReplaceAllUnchecked(t1, new H.JSSyntaxRegExp(t2, null, null), "I") + "ABCDEFGHIKLMNOPQRSTUVWXYZ";
    P.print("Key is " + newKey);
    this.alphabet = P.List_List(null, null);
    for (t1 = playfairKey.length, i = 0; i < t1; ++i)
      if (!C.JSArray_methods.contains$1(this.alphabet, playfairKey[i]))
        this.alphabet.push(playfairKey[i]);
    for (t1 = this.columns, t2 = this.rows, count = 0, j = 0; t3 = this.alphabet, j < t3.length; ++j) {
      if (C.JSInt_methods.$mod(j / 5, 1) === 0)
        ++count;
      t4 = C.JSInt_methods.$mod(j, 5);
  ...
  cryptPair$2: function(pair, encrypt) {
    var direction, t1, letter1, letter2, l1Row, l1Col, l2Row, l2Col, t2, t3, t4, newLetter1, newLetter2;
    direction = encrypt ? 1 : -1;
    t1 = pair.length;
    if (0 >= t1)
      throw H.ioore(pair, 0);
    letter1 = pair[0];
    if (1 >= t1)
      throw H.ioore(pair, 1);
    letter2 = pair[1];
    l1Row = this.getRow$1(letter1);
    l1Col = this.getColumn$1(letter1);
    l2Row = this.getRow$1(letter2);
    l2Col = this.getColumn$1(letter2);
    if (l1Row === l2Row) {
      t1 = C.JSNumber_methods.$mod(5 + (l1Col + direction), 5);
      t2 = this.rows;
      t3 = t2.length;
      if (l1Row >>> 0 !== l1Row || l1Row >= t3)
        throw H.ioore(t2, l1Row);
      t4 = t2[l1Row];
      if (t1 >>> 0 !== t1 || t1 >= t4.length)
        throw H.ioore(t4, t1);
      newLetter1 = t4[t1];
      t1 = C.JSNumber_methods.$mod(5 + (l2Col + direction), 5);
      if (l2Row >>> 0 !== l2Row || l2Row >= t3)
        throw H.ioore(t2, l2Row);
      t2 = t2[l2Row];
      ...
      t1 = this.rows;
      if (l1Col === l2Col) {
        t2 = C.JSNumber_methods.$mod(5 + (l1Row + direction), 5);
        t3 = t1.length;
        if (t2 >>> 0 !== t2 || t2 >= t3)
          throw H.ioore(t1, t2);
        t2 = t1[t2];
        if (l1Col >>> 0 !== l1Col || l1Col >= t2.length)
          throw H.ioore(t2, l1Col);
        newLetter1 = t2[l1Col];
        t2 = C.JSNumber_methods.$mod(5 + (l2Row + direction), 5);
        if (t2 >>> 0 !== t2 || t2 >= t3)
          throw H.ioore(t1, t2);
        t2 = t1[t2];
        if (l2Col >>> 0 !== l2Col || l2Col >= t2.length)
          throw H.ioore(t2, l2Col);
        newLetter2 = t2[l2Col];
      } else {
        t2 = t1.length;
        if (l1Row >>> 0 !== l1Row || l1Row >= t2)
          throw H.ioore(t1, l1Row);
        t3 = t1[l1Row];
        if (l2Col >>> 0 !== l2Col || l2Col >= t3.length)
          throw H.ioore(t3, l2Col);
        newLetter1 = t3[l2Col];
        if (l2Row >>> 0 !== l2Row || l2Row >= t2)
          throw H.ioore(t1, l2Row);
        t1 = t1[l2Row];
        if (l1Col >>> 0 !== l1Col || l1Col >= t1.length)
          throw H.ioore(t1, l1Col);
        newLetter2 = t1[l1Col];
      }
    }
    return J.$add$ns(newLetter1, newLetter2);
  },
  getRow$1: function(letter) {
    var t1, count, letterpos;
    t1 = this.rows;
    count = 0;
    do {
      if (count >= t1.length)
        throw H.ioore(t1, count);
      letterpos = C.JSArray_methods.contains$1(t1[count], letter) ? count : -1;
      ++count;
    } while (letterpos < 0);
    return letterpos;
  },
  getColumn$1: function(letter) {
    var t1, count, letterpos;
    t1 = this.columns;
    count = 0;
    do {
  ...
  Square$1: function(key) {
    var t1, t2, i;
    for (t1 = this.columns, t2 = this.rows, i = 0; i < 5; ++i) {
      t1.push(P.List_List(null, null));
      t2.push(P.List_List(null, null));
    }
    this.generate$1(key);
  },
  static: {
Square$: function(key) {
  var t1 = new O.Square(P.List_List(null, null), P.List_List(null, null), P.List_List(null, null));
  t1.Square$1(key);
  return t1;
}}
...
init.globalFunctions.preCrypt$closure = O.preCrypt$closure = new P.Closure$1(O.preCrypt, "preCrypt$closure");
init.globalFunctions.changeKey$closure = O.changeKey$closure = new P.Closure$1(O.changeKey, "changeKey$closure");
init.globalFunctions.upperCaseKey$closure = O.upperCaseKey$closure = new P.Closure$1(O.upperCaseKey, "upperCaseKey$closure");
...
function dart_precompiled($collectedClasses) {
  ...
  function Square(alphabet, columns, rows) {
    this.alphabet = alphabet;
    this.columns = columns;
    this.rows = rows;
  }
  Square.builtin$cls = "Square";
  if (!"name" in Square)
    Square.name = "Square";
  $desc = $collectedClasses.Square;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Square.prototype = $desc;
